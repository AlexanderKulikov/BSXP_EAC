...
Процедура ОтладкаЖивотноводство2019(Объект, НачальнаяТаблица, СтруктураСтроки, ВыборкаПолейШаблона) Экспорт
	...
	Для НомерГода = 2019 По 2020 Цикл
		...
		ПодразделениеКРС = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Подразделение: Животноводство КРС");
		ВидыНоменклатураПродуктивныйСкот = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "ВидыНоменклатуры: Животные продуктивный скот");
		...
		|		(&СчетУчетаЖивотных), &ВидыСубконто, Организация = &Организация) КАК ХозрасчетныйОстаткиИОбороты
		|ГДЕ
		|	&СкладскойУчет > 0
		|	И (&ВариантОтчета = 0
		|	ИЛИ &ВариантОтчета = 2)";
		...
		|		, &ВидыСубконтоБезСклада, Организация = &Организация) КАК ХозрасчетныйОстаткиИОбороты
		|ГДЕ
		|	&СкладскойУчет = 0
		|	И (&ВариантОтчета = 0
		|	ИЛИ &ВариантОтчета = 2)";
		...
		ЗапросСП51.Текст = "ВЫБРАТЬ
		                   ...
		                   |		ЖивотныеНаВыращивании.Подразделение КАК Подразделение,
		                   ...
		                   |		ЖивотныеНаВыращивании.Подразделение
		                   |	
		                   |	ОБЪЕДИНИТЬ ВСЕ
		                   |	
		                   |	ВЫБРАТЬ
		                   ...
		                   |		ЖивотныеНаВыращиванииБезСклада.Подразделение,
		                   ...
		                   |	СГРУППИРОВАТЬ ПО
		                   |		ЖивотныеНаВыращиванииБезСклада.Номенклатура,
		                   |		ЖивотныеНаВыращиванииБезСклада.Подразделение) КАК ВложенныйЗапрос
						   |	ГДЕ 
						   |		ВложенныйЗапрос.Номенклатура.ВидНоменклатуры В (&ВидыНоменклатуры)
		...
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ГоловКонечныйОстаток, Ложь);
		
		// Приплод
		СтруктураСтроки.ДопПараметр         = НСтр("ru = 'Получено приплода'");  
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ГоловПриплод, Ложь);
		...
		ЗапросСП51.Текст = "ВЫБРАТЬ
		                   ...
		                   |		И ВложенныйЗапрос.Номенклатура = &Корова";
		НоменклатураКорова = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Номенклатура Корова");
		...
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, КоровКонечныйОстаток, Ложь);
		...
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, СреднегодовоеПоголовьеКоров, Ложь);
		...
		ЗапросСП51.Текст = "ВЫБРАТЬ
		                   ...
		                   |ГДЕ
						   |	ВложенныйЗапрос.Подразделение = &Подразделение
		                   |	И ВложенныйЗапрос.Номенклатура В (&Коровы)";
		НоменклатураНетели = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Номенклатура Нетели");
		...
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ДеловойВыходТелят, Ложь);
		...
		НоменклатураМолоко = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Номенклатура Молоко");
		Запрос = Новый Запрос;
		...
		Пока Выборка.Следующий() Цикл
			...
			ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ИтогоКоличествоУдоя/100, Ложь);
		КонецЦикла;
		...
		КонецЕсли;	
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, СреднегодовойУдойНаКорову, Истина);
		...
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ?(ИтогоКоличество = 0, 0, ИтогоСумма/ИтогоКоличество), Ложь);
		...
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ИтогоРеализованоМолока/100, Ложь);
		
		// Выручка от реализации молока
		НГМолоко = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Номенклатурная группа Молоко");
		...
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ИтогоВыручкаМолока/1000, Ложь);
		...
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ИтогоСебестоимостьМолока / 1000, Ложь);
		
		// Прибыль/убыток от продажи молока
		СтруктураСтроки.ДопПараметр      = НСтр("ru = 'Прибыль/убыток от продажи молока'");
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, (ИтогоВыручкаМолока - ИтогоСебестоимостьМолока) / 1000, Ложь);
		
		
		// ПРОИЗВОДСТВО МЯСА
		ПоказательПроизводствоМяса = Справочники.ЕАС_ПоказателиОтчетов.НайтиПоНаименованию("Производство мяса", ,ГруппаЖивотноводство ,Объект.Шаблон);
		ЗапросСП51.Текст = "ВЫБРАТЬ
		                   |		ЖивотныеНаВыращивании.Номенклатура КАК Номенклатура,
		...
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, МассаПривес, Ложь);
		
		// Произведено мяса КРС
		СтруктураСтроки.ДопПараметр = НСтр("ru = 'Произведено мяса КРС'");
		ПоказателиАПК = ЕАС_ОбщегоНазначенияЛокальный.ПолучитьПоказателиАПК(ПериодАПК, Объект.Организация, "СХ", "Форма14АПК", "стр142221_гр3", 0);
		ПроизведеноМясаКРС = 0;
		ПоказателиАПК.Свойство("Форма14АПК_стр142221_гр3", ПроизведеноМясаКРС);
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ПроизведеноМясаКРС, Ложь);

		// Реализовано мяса КРС
		СтруктураСтроки.ДопПараметр      = НСтр("ru = 'Реализовано мяса КРС'");
		ПоказателиАПК = ЕАС_ОбщегоНазначенияЛокальный.ПолучитьПоказателиАПК(ПериодАПК, Объект.Организация, "СХ", "Форма14АПК", "стр143221_гр3", 0);
		РеализованоМясаКРС = 0;
		ПоказателиАПК.Свойство("Форма14АПК_стр143221_гр3", РеализованоМясаКРС);
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, РеализованоМясаКРС, Ложь);
		
		// Себестоимость мяса
		СтруктураСтроки.ДопПараметр      = НСтр("ru = 'Себестоимость мяса'");
		СебестоимостьМясаКРС = 0;
		ПоказателиАПК = ЕАС_ОбщегоНазначенияЛокальный.ПолучитьПоказателиАПК(ПериодАПК, Объект.Организация, "СХ", "Форма14АПК", "стр142221_гр13", 0);
		ПоказателиАПК.Свойство("Форма14АПК_стр142221_гр13", СебестоимостьМясаКРС);
		СтруктураСтроки.ЕдиницаИзмерения = ПустаяЕИ;
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, СебестоимостьМясаКРС/100, Ложь);
		
		// Выручка от реализации мяса
		СтруктураСтроки.ДопПараметр      = НСтр("ru = 'Выручка от реализации мяса'");
		ИтогоВыручкаМяса = 0;
		ПоказателиАПК = ЕАС_ОбщегоНазначенияЛокальный.ПолучитьПоказателиАПК(ПериодАПК, Объект.Организация, "СХ", "Форма14АПК", "стр143221_гр7", 0);
		ПоказателиАПК.Свойство("Форма14АПК_стр143221_гр7", ИтогоВыручкаМяса);
		СтруктураСтроки.ЕдиницаИзмерения = ТысячаРублей;
		Если НомерГода = 2019 Тогда
			ИтогоВыручкаМяса = ИтогоВыручкаМяса / 1000; // в 2019 подавались рубли
		КонецЕсли;	
		СтрокаВыручки = ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ИтогоВыручкаМяса, Ложь);
		
		// Себестоимость продажи мяса
		СтруктураСтроки.ДопПараметр      = НСтр("ru = 'Себестоимость продажи мяса'");
		ИтогоСебестоимостьМяса = 0;
		ПоказателиАПК = ЕАС_ОбщегоНазначенияЛокальный.ПолучитьПоказателиАПК(ПериодАПК, Объект.Организация, "СХ", "Форма14АПК", "стр143221_гр5", 0);
		ПоказателиАПК.Свойство("Форма14АПК_стр143221_гр5", ИтогоСебестоимостьМяса);
		Если НомерГода = 2019 Тогда
			ИтогоСебестоимостьМяса = ИтогоСебестоимостьМяса / 1000; // в 2019 подавались рубли
		КонецЕсли;	
		СтрокаСебестоимость = ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ИтогоСебестоимостьМяса, Ложь);
		
		// Прибыль/убыток от продажи мяса
		ЕАС_ЗаполнениеОтчетовКопируемый.ФорматированиеЗначенийВычислений(НачальнаяТаблица, СтруктураСтроки, ВыборкаПолейШаблона);
		СтруктураСтроки.ДопПараметр      = НСтр("ru = 'Прибыль/убыток от продажи мяса'");
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, СтрокаВыручки.РезультатВычисления - СтрокаСебестоимость.РезультатВычисления, Ложь);
		...
		ОбеспечениеСкотоводства = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Назначение средств: Обеспечение скотоводства"); 
		//ТысячаРублей = Справочники.КлассификаторЕдиницИзмерения.НайтиПоКоду("384");
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ
		               |	ХозрасчетныйОбороты.Счет КАК Счет,
		...
		МассивНазначений.Добавить(ОбеспечениеСкотоводства);
		...
		Пока Выборка.Следующий() Цикл
			...
			ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, Выборка.СуммаОборотДт/1000, Истина);
			ИтогоСумма = ИтогоСумма + Выборка.СуммаОборотДт/1000;
		КонецЦикла;
		
		// Среднегодовая численность работников скотоводства
		ПоказательСреднегодоваяЧисленность = Справочники.ЕАС_ПоказателиОтчетов.НайтиПоНаименованию("Среднегодовая численность работников скотоводства", ,ГруппаЖивотноводство, Объект.Шаблон);
		СтруктураСтроки.ПоказательОтчета 	= ПоказательСреднегодоваяЧисленность;
		СтруктураСтроки.Источник            = Перечисления.ЕАС_ВидыИсточниковДанных.Факт;
		СтруктураСтроки.ЕдиницаИзмерения    = ПустаяЕИ;
		СтруктураСтроки.ДатаНачала          = ДатаНачала;
		СтруктураСтроки.ДатаОкончания       = ДатаОкончания;
		СтруктураСтроки.ДопПараметр         = "";
		СреднегодоваяЧисленность_стр51112 = 0;
		ПоказателиАПК = ЕАС_ОбщегоНазначенияЛокальный.ПолучитьПоказателиАПК(ПериодАПК, Объект.Организация, "СХ", "Форма5АПК", "стр51112_гр3", 0);
		ПоказателиАПК.Свойство("Форма5АПК_стр51112_гр3", СреднегодоваяЧисленность_стр51112);
		СреднегодоваяЧисленность_стр51113 = 0;
		ПоказателиАПК = ЕАС_ОбщегоНазначенияЛокальный.ПолучитьПоказателиАПК(ПериодАПК, Объект.Организация, "СХ", "Форма5АПК", "стр51113_гр3", 0);
		ПоказателиАПК.Свойство("Форма5АПК_стр51113_гр3", СреднегодоваяЧисленность_стр51113);
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, СреднегодоваяЧисленность_стр51112 + СреднегодоваяЧисленность_стр51113, Ложь);
		
		
		ПоказательСреднеМесячнаяЗарплата = Справочники.ЕАС_ПоказателиОтчетов.НайтиПоНаименованию("Среднемесячная заработная плата на 1 работника скотоводства", ,ГруппаЖивотноводство, Объект.Шаблон);
		СтруктураСтроки.ПоказательОтчета 	= ПоказательСреднеМесячнаяЗарплата;
		СтруктураСтроки.Источник            = Перечисления.ЕАС_ВидыИсточниковДанных.Факт;
		СтруктураСтроки.ЕдиницаИзмерения    = ТысячаРублей;
		СтруктураСтроки.ДатаНачала          = ДатаНачала;
		СтруктураСтроки.ДатаОкончания       = ДатаОкончания;
		СтруктураСтроки.ДопПараметр         = "";
		ГодоваяЗарплата_стр51112 = 0;
		ПоказателиАПК = ЕАС_ОбщегоНазначенияЛокальный.ПолучитьПоказателиАПК(ПериодАПК, Объект.Организация, "СХ", "Форма5АПК", "стр51112_гр4", 0);
		ПоказателиАПК.Свойство("Форма5АПК_стр51112_гр4", ГодоваяЗарплата_стр51112);
		ГодоваяЗарплата_стр51113 = 0;
		ПоказателиАПК = ЕАС_ОбщегоНазначенияЛокальный.ПолучитьПоказателиАПК(ПериодАПК, Объект.Организация, "СХ", "Форма5АПК", "стр51113_гр4", 0);
		ПоказателиАПК.Свойство("Форма5АПК_стр51113_гр4", ГодоваяЗарплата_стр51113);
		Если (СреднегодоваяЧисленность_стр51112 + СреднегодоваяЧисленность_стр51113) <> 0 Тогда
			СреднеМесячнаяЗарплата = (ГодоваяЗарплата_стр51112 + ГодоваяЗарплата_стр51113) / (СреднегодоваяЧисленность_стр51112 + СреднегодоваяЧисленность_стр51113) / 12;
		Иначе
			СреднеМесячнаяЗарплата = 0;
		КонецЕсли;		 
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, СреднеМесячнаяЗарплата, Ложь);
		
		
		
		/////////////////////////////////////////////
		// ЛОШАДИ / КОБЫЛЫ //////////////////////////
		ПодразделениеКоневодство = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Подразделение: Коневодство");
		ВидыНоменклатураПродуктивныеЛошади = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "ВидыНоменклатуры: Животные продуктивные Лошади");
		...
		ЗапросСП51.Текст = "ВЫБРАТЬ
		                   ...
		                   |	ВЫБРАТЬ
		                   |		ЖивотныеНаВыращиванииБезСклада.Номенклатура,
		                   |		ЖивотныеНаВыращиванииБезСклада.Подразделение КАК Подразделение,
		                   |		СУММА(ЖивотныеНаВыращиванииБезСклада.ГоловКонечныйОстаток),
		                   |		СУММА(ЖивотныеНаВыращиванииБезСклада.ГоловПриплод) КАК ГоловПриплод
		                   |	ИЗ
		                   |		ЖивотныеНаВыращиванииБезСклада КАК ЖивотныеНаВыращиванииБезСклада
						   ...
						   |		ВложенныйЗапрос.Номенклатура.ВидНоменклатуры В (&ВидыНоменклатуры)
						   |		И ВложенныйЗапрос.Подразделение = &Подразделение";
		ЗапросСП51.УстановитьПараметр("ВидыНоменклатуры",  ВидыНоменклатураПродуктивныеЛошади);
		ЗапросСП51.УстановитьПараметр("Подразделение",     ПодразделениеКоневодство);
		РезультатЗапроса = ЗапросСП51.Выполнить().Выбрать();
		ЕстьРезультат = РезультатЗапроса.Следующий();           
		Если ЕстьРезультат И ЗначениеЗаполнено(РезультатЗапроса.ГоловКонечныйОстаток) Тогда
			ГоловКонечныйОстаток = РезультатЗапроса.ГоловКонечныйОстаток;
		Иначе	
			ГоловКонечныйОстаток = 0;
		КонецЕсли;	
		Если ЕстьРезультат И ЗначениеЗаполнено(РезультатЗапроса.ГоловПриплод) Тогда
			ГоловПриплод = РезультатЗапроса.ГоловПриплод;
		Иначе
			ГоловПриплод = 0;
		КонецЕсли;	
		...
		Конематки = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Номенклатура Конематки");
		ЗапросСП51.Текст = "ВЫБРАТЬ
		                   |	СУММА(ВложенныйЗапрос.ГоловКонечныйОстаток) КАК ГоловКонечныйОстаток
		                   |ИЗ
		                   |	(ВЫБРАТЬ
		                   ...
		                   |		ЖивотныеНаВыращиванииБезСклада.Подразделение) КАК ВложенныйЗапрос
		                   |ГДЕ
		                   |	ВложенныйЗапрос.Номенклатура в (&Конематки)
						   |		И ВложенныйЗапрос.Подразделение = &Подразделение";
		ЗапросСП51.УстановитьПараметр("Конематки", Конематки);    
		РезультатЗапроса = ЗапросСП51.Выполнить().Выбрать();
		Если РезультатЗапроса.Следующий() Тогда
		...
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ЛошадиКонечныйОстаток, Ложь);
		
		// Среднегодовое поголовье лошадей (из Формы АПК 13)
		//		СтруктураСтроки.ДопПараметр = НСтр("ru = 'Среднегодовое поголовье конематок'");
		//		ПоказателиАПК = ЕАС_ОбщегоНазначенияЛокальный.ПолучитьПоказателиАПК(ПериодАПК, Объект.Организация, "СХ", "Форма13АПК", "стр131110_гр3", 0);
		//		СреднегодовоеПоголовьеКонематок = 0;
		//		ПоказателиАПК.Свойство("Форма13АПК_стр131110_гр3", СреднегодовоеПоголовьеКонематок);
		//		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, СреднегодовоеПоголовьеКонематок, Ложь);
		...
		ЗапросСП51.Текст = "ВЫБРАТЬ
		                   |	СУММА(ВложенныйЗапрос.ГоловНачальныйОстаток) КАК ГоловНачальныйОстаток,
		                   |	СУММА(ВложенныйЗапрос.ГоловПереведеноИзДругихГрупп) КАК ГоловПереведеноИзДругихГрупп
		                   |ИЗ
		                   |	(ВЫБРАТЬ
		                   |		ЖивотныеНаВыращивании.Номенклатура КАК Номенклатура,
		                   |		ЖивотныеНаВыращивании.Подразделение КАК Подразделение,
		                   |		СУММА(ЖивотныеНаВыращивании.ГоловНачальныйОстаток) КАК ГоловНачальныйОстаток,
		                   |		СУММА(ЖивотныеНаВыращивании.ГоловПереведеноИзДругихГрупп) КАК ГоловПереведеноИзДругихГрупп
		                   |	ИЗ
		                   |		ЖивотныеНаВыращивании КАК ЖивотныеНаВыращивании
		                   |	
		                   |	СГРУППИРОВАТЬ ПО
		                   ...
		                   |		ОсновныеСредства.Подразделение,
		                   |		СУММА(ОсновныеСредства.ГоловНачальныйОстаток),
		                   |		СУММА(ОсновныеСредства.ГоловПереведеноИзДругихГрупп)
		                   |	ИЗ
		                   |		ОсновныеСредства КАК ОсновныеСредства
		                   |	
		                   |	СГРУППИРОВАТЬ ПО
		                   ...
		                   |		ЖивотныеНаВыращиванииБезСклада.Подразделение,
		                   |		СУММА(ЖивотныеНаВыращиванииБезСклада.ГоловНачальныйОстаток),
		                   |		СУММА(ЖивотныеНаВыращиванииБезСклада.ГоловПереведеноИзДругихГрупп)
		                   |	ИЗ
		                   |		ЖивотныеНаВыращиванииБезСклада КАК ЖивотныеНаВыращиванииБезСклада
		                   |	
		                   |	СГРУППИРОВАТЬ ПО
		...
		РезультатЗапроса = ЗапросСП51.Выполнить().Выбрать();
		ЕстьРезультат = РезультатЗапроса.Следующий();           
		Если ЕстьРезультат И ЗначениеЗаполнено(РезультатЗапроса.ГоловНачальныйОстаток) Тогда
			НачальныйОстатокЛошади = РезультатЗапроса.ГоловНачальныйОстаток;
		Иначе	
			НачальныйОстатокЛошади = 0;
		КонецЕсли;	 
		Если ЕстьРезультат И ЗначениеЗаполнено(РезультатЗапроса.ГоловПереведеноИзДругихГрупп) Тогда
			ГоловПереведеноИзДругихГрупп = РезультатЗапроса.ГоловПереведеноИзДругихГрупп; 
		Иначе	
			ГоловПереведеноИзДругихГрупп = 0;
		КонецЕсли;	
		Если (НачальныйОстатокЛошади + ГоловПереведеноИзДругихГрупп) = 0 Тогда
			ДеловойВыходЖеребят = ?(ГоловПриплод = 0, 0, 100);      
		Иначе	
			ДеловойВыходЖеребят = ГоловПриплод/(НачальныйОстатокЛошади + ГоловПереведеноИзДругихГрупп) * 100;
		КонецЕсли;	
		...
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ДеловойВыходЖеребят, Ложь);
		...
		//		КонецЕсли;	
		//		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, СреднегодовойУдойНаКобылу, Истина);
		//		
		//		
		...
		//		СтруктураСтроки.ЕдиницаИзмерения    = Центнер;
		//		СтруктураСтроки.ДопПараметр = НСтр("ru = 'Реализовано молока'");
		//		СтрокаРеализовано = ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ИтогоРеализованоМолока/100, Ложь);
		//		
		...
		//		СтруктураСтроки.ЕдиницаИзмерения = ТысячаРублей;
		//		СтруктураСтроки.ДопПараметр      = НСтр("ru = 'Выручка от реализации молока'");
		//		СтрокаВыручки = ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ИтогоВыручкаМолокаКобыл/1000, Ложь);
		//		
		...
		//		СтруктураСтроки.ЕдиницаИзмерения = ТысячаРублей;
		//		СтруктураСтроки.ДопПараметр      = НСтр("ru = 'Себестоимость продажи молока'");
		//		СтрокаСебестоимость = ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ИтогоСебестоимостьМолокаКобыл / 1000, Ложь);   
		//		
		//		// Прибыль/убыток от продажи молока кобыл
		//		СтруктураСтроки.ДопПараметр      = НСтр("ru = 'Прибыль/убыток от продажи молока'");
		//		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, (СтрокаВыручки.РезультатВычисления - СтрокаСебестоимость.РезультатВычисления) / 1000, Ложь);
		
		
		// ПРОИЗВОДСТВО МЯСА ЛОШАДЕй
		// Получено прироста молодняка лошадей в живом весе
		...
		//ПродукцияМясоЛошадей = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Номенклатура Мясо конина");
		ЗапросСП51.Текст = "ВЫБРАТЬ
		                   ...
		                   |		ОсновныеСредства.Номенклатура,
		                   |		ОсновныеСредства.Подразделение,
		                   |		СУММА(ОсновныеСредства.МассаПривес)
		                   |	ИЗ
		                   |		ОсновныеСредства КАК ОсновныеСредства
		                   |	
		                   |	СГРУППИРОВАТЬ ПО
		...
		ПоказателиАПК = ЕАС_ОбщегоНазначенияЛокальный.ПолучитьПоказателиАПК(ПериодАПК, Объект.Организация, "СХ", "Форма14АПК", "стр142225_гр3", 0);
		ИтогоРеализованоМясаЛошадей = 0;
		ПоказателиАПК.Свойство("Форма14АПК_стр142225_гр3", ИтогоРеализованоМясаЛошадей);
		СтруктураСтроки.ЕдиницаИзмерения = Центнер;
		СтруктураСтроки.ДопПараметр      = НСтр("ru = 'Реализовано мяса лошадей'");
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ИтогоРеализованоМясаЛошадей, Ложь);
		
		// Себестоимость мяса
		ПоказателиАПК = ЕАС_ОбщегоНазначенияЛокальный.ПолучитьПоказателиАПК(ПериодАПК, Объект.Организация, "СХ", "Форма14АПК", "стр142225_гр13", 0);
		СебестоимостьМясаЛошадей = 0;
		ПоказателиАПК.Свойство("Форма14АПК_стр142225_гр13", СебестоимостьМясаЛошадей);
		
		//Рубль = Справочники.КлассификаторЕдиницИзмерения.НайтиПоКоду("383");
		СтруктураСтроки.ДопПараметр       = НСтр("ru = 'Себестоимость мяса'");
		СтруктураСтроки.ЕдиницаИзмерения  = ПустаяЕИ;
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, СебестоимостьМясаЛошадей/100, Ложь);
		
		// Выручка от реализации мяса конины
		ПоказателиАПК = ЕАС_ОбщегоНазначенияЛокальный.ПолучитьПоказателиАПК(ПериодАПК, Объект.Организация, "СХ", "Форма14АПК", "стр143225_гр7", 0);
		ИтогоВыручкаМясаЛошадей = 0;
		ПоказателиАПК.Свойство("Форма14АПК_стр143225_гр7", ИтогоВыручкаМясаЛошадей);
		Если НомерГода = 2019 Тогда
			ИтогоВыручкаМясаЛошадей = ИтогоВыручкаМясаЛошадей / 1000;
		КонецЕсли;	
		СтруктураСтроки.ЕдиницаИзмерения = ТысячаРублей;
		СтруктураСтроки.ДопПараметр      = НСтр("ru = 'Выручка от реализации мяса лошадей'");
		СтрокаВыручки = ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ИтогоВыручкаМясаЛошадей, Ложь);
		
		// Себестоимость продажи мяса конины
		ПоказателиАПК = ЕАС_ОбщегоНазначенияЛокальный.ПолучитьПоказателиАПК(ПериодАПК, Объект.Организация, "СХ", "Форма14АПК", "стр143225_гр5", 0);
		ИтогоСебестоимостьМясаКонины = 0;
		ПоказателиАПК.Свойство("Форма14АПК_стр143225_гр5", ИтогоСебестоимостьМясаКонины);
		Если НомерГода = 2019 Тогда
			ИтогоСебестоимостьМясаКонины = ИтогоСебестоимостьМясаКонины / 1000;
		КонецЕсли;	
		СтруктураСтроки.ЕдиницаИзмерения = ТысячаРублей;
		СтруктураСтроки.ДопПараметр      = НСтр("ru = 'Себестоимость продажи мяса лошадей'");
		СтрокаСебестоимость = ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ИтогоСебестоимостьМясаКонины, Ложь);
		...
		// Среднегодовая численность прочего эивотновожства
		ПоказательСреднегодоваяЧисленность = Справочники.ЕАС_ПоказателиОтчетов.НайтиПоНаименованию("Субсидия на содержание кобыл", ,ГруппаЛошади, Объект.Шаблон);
		СтруктураСтроки.ПоказательОтчета 	= ПоказательСреднегодоваяЧисленность;
		СтруктураСтроки.Источник            = Перечисления.ЕАС_ВидыИсточниковДанных.Факт;
		СтруктураСтроки.ЕдиницаИзмерения    = ПустаяЕИ;
		СтруктураСтроки.ДатаНачала          = ДатаНачала;
		СтруктураСтроки.ДатаОкончания       = ДатаОкончания;
		СтруктураСтроки.ДопПараметр         = Нстр("ru = 'Среднегодовая численность работников прочего живот'");
		СреднегодоваяЧисленность_стр51117 = 0;
		ПоказателиАПК = ЕАС_ОбщегоНазначенияЛокальный.ПолучитьПоказателиАПК(ПериодАПК, Объект.Организация, "СХ", "Форма5АПК", "стр51117_гр3", 0);
		ПоказателиАПК.Свойство("Форма5АПК_стр51117_гр3", СреднегодоваяЧисленность_стр51117);
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, СреднегодоваяЧисленность_стр51117, Ложь);
		
		
		СтруктураСтроки.ЕдиницаИзмерения    = ТысячаРублей;
		СтруктураСтроки.ДопПараметр         = Нстр("ru = 'Среднемесячная заработная плата на 1 работника'");
		ГодоваяЗарплата_стр51117 = 0;
		ПоказателиАПК = ЕАС_ОбщегоНазначенияЛокальный.ПолучитьПоказателиАПК(ПериодАПК, Объект.Организация, "СХ", "Форма5АПК", "стр51117_гр4", 0);
		ПоказателиАПК.Свойство("Форма5АПК_стр51117_гр4", ГодоваяЗарплата_стр51117);
		Если СреднегодоваяЧисленность_стр51117 <> 0 Тогда
			СреднеМесячнаяЗарплата = ГодоваяЗарплата_стр51117 / СреднегодоваяЧисленность_стр51117 / 12;
		Иначе
			СреднеМесячнаяЗарплата = 0;
		КонецЕсли;		 
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, СреднеМесячнаяЗарплата, Ложь);
		...
		СтрокаВыручка = ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, 0, Истина);
		Если СтрокиМолоко.Количество() > 0 Тогда
			ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, СтрокиМолоко[0].РезультатВычисления, Истина);
		КонецЕсли;
		Если СтрокиМолокоКобыл.Количество() > 0 Тогда
			ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, СтрокиМолокоКобыл[0].РезультатВычисления, Истина);
		КонецЕсли;
		Если СтрокиМясо.Количество() > 0 Тогда
			ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, СтрокиМясо[0].РезультатВычисления, Истина);
		КонецЕсли;
		Если СтрокиМясоЛошадей.Количество() > 0 Тогда
			ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, СтрокиМясоЛошадей[0].РезультатВычисления, Истина);
		КонецЕсли;
		...
		СтрокаСебестоимость = ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, 0, Истина);
		Если СтрокиМолоко.Количество() > 0 Тогда
			ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, СтрокиМолоко[0].РезультатВычисления, Истина);
		КонецЕсли;
		Если СтрокиМолокоКобыл.Количество() > 0 Тогда
		...
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, СтрокаВыручка.РезультатВычисления - СтрокаСебестоимость.РезультатВычисления, Истина);
...
Процедура ОтладкаЗерноводствоКормозаготовка2019(Объект, НачальнаяТаблица, СтруктураСтроки, ВыборкаПолейШаблона) Экспорт
	...
	Для НомерГода = 2019 По 2020 Цикл
		...
		ПодразделениеЗерноводство   = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "ПодразделениеЗерноводство");
		ПодразделениеКормозаготовка = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "ПодразделениеКормозаготовка");
		
		НомГруппаОвес    = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Ном группа: Овес");
		НомГруппаПшеница = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Ном группа: Пшеница");
		НомГруппаЯчмень  = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Ном группа: Ячмень");
		НомГруппаПрочееЗерноводство  = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Ном группа: Прочее зерноводство");
		
		НоменклатураОвес    = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Номенклатура: Овес");
		НоменклатураПшеница = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Номенклатура: Пшеница");
		НоменклатураЯчмень  = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Номенклатура: Ячмень");

		КультураОвес    = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Культура: Овес");
		КультураПшеница = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Культура: Пшеница");
		КультураЯчмень  = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Культура: Ячмень");
		...
		ОписаниеСправочников.Вставить("Зерноводство",   Новый Структура("НоменклатурныеГруппы, Продукты, Культуры, СтруктурыПосева, Идентификаторы", Новый Массив, Новый Массив, Новый Массив, Новый Массив, Новый Массив));
		ОписаниеСправочников.Вставить("Кормозаготовка", Новый Структура("НоменклатурныеГруппы, Продукты, Культуры, СтруктурыПосева, Идентификаторы", Новый Массив, Новый Массив, Новый Массив, Новый Массив, Новый Массив));
		Пока Выборка.Следующий() Цикл
			...
			Иначе
				Продолжить;
			КонецЕсли;	
			Идентификатор = НСтр("ru = 'Прочие'");
			Если Выборка.ПодразделениеЗатрат = ПодразделениеЗерноводство Тогда
				Если Выборка.НоменклатурнаяГруппа = НомГруппаОвес Тогда
					Идентификатор = НСтр("ru = 'Овес'");
				ИначеЕсли Выборка.НоменклатурнаяГруппа = НомГруппаПшеница Тогда
					Идентификатор = НСтр("ru = 'Пшеница'");
				ИначеЕсли Выборка.НоменклатурнаяГруппа = НомГруппаЯчмень Тогда
					Идентификатор = НСтр("ru = 'Ячмень'");
				КонецЕсли;
			КонецЕсли;		
			...
			СтруктураСправочника.Идентификаторы.Добавить(Идентификатор);
		КонецЦикла;	
		...
		Идентификаторы 	 	 = СтруктураСправочника.Идентификаторы;
		
		ГруппаЗерноводство  = Справочники.ЕАС_ПоказателиОтчетов.НайтиПоНаименованию("Зерноводство", , Справочники.ЕАС_ПоказателиОтчетов.ПустаяСсылка(), Объект.Шаблон);
		...
		Пока Выборка.Следующий() Цикл
			...
			Если КультураОвес.Найти(Выборка.Культура) <> Неопределено Тогда
				СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Овес'");
			ИначеЕсли КультураПшеница.Найти(Выборка.Культура) <> Неопределено Тогда
				СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Пшеница'");
			ИначеЕсли КультураЯчмень.Найти(Выборка.Культура) <> Неопределено Тогда
				СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Ячмень'");
			Иначе				
				СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Прочие'");
			КонецЕсли;
			
			ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, Выборка.ПлощадьПоля, Истина);
			ИтогоПлощадь = ИтогоПлощадь + Выборка.ПлощадьПоля;
		КонецЦикла;
		СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Итого'");
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ИтогоПлощадь, Ложь);
		...
		Пока Выборка.Следующий() Цикл                  
			...
			Если НоменклатураОвес.Найти(Выборка.Продукция) <> Неопределено Тогда
				СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Овес'");
			ИначеЕсли НоменклатураПшеница.Найти(Выборка.Продукция) <> Неопределено Тогда
				СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Пшеница'");
			ИначеЕсли НоменклатураЯчмень.Найти(Выборка.Продукция) <> Неопределено Тогда
				СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Ячмень'");
			Иначе				
				СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Прочие'");
			КонецЕсли;

			ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, Выборка.Количество, Истина);
			ИтогоКоличество = ИтогоКоличество + Выборка.Количество;
		КонецЦикла;
		СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Итого'");
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ИтогоКоличество, Ложь);
		
		// СЕБЕСТОИМОСТЬ ЕДИНИЦЫ УРОЖАЯ
		НоменклатураКорма    = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Номенклатура: Корма");
		...
		Запрос.УстановитьПараметр("НачалоПериода", ДатаНачала);
		Запрос.УстановитьПараметр("Организация",   Объект.Организация);
		Запрос.УстановитьПараметр("Корма",         НоменклатураКорма);
		...
		Пока Выборка.Следующий() Цикл
			...
			СтрокаТаблицы = ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, 0, Истина);
		...
		Пока Выборка.Следующий() Цикл
			...
			ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, Выборка.КоличествоОборотКт, Истина);
		...
		Пока Выборка.Следующий() Цикл
			...
			Если НомГруппаОвес = Выборка.Продукция Тогда
				СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Овес'");
			ИначеЕсли НомГруппаПшеница = Выборка.Продукция Тогда
				СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Пшеница'");
			ИначеЕсли НомГруппаЯчмень = Выборка.Продукция Тогда
				СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Ячмень'");
			ИначеЕсли НомГруппаПрочееЗерноводство = Выборка.Продукция  
						Или НоменклатурныеГруппы.Найти(Выборка.Продукция) <> Неопределено Тогда
				СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Прочие'");
			Иначе				
				Продолжить;
			КонецЕсли;

			ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, Выборка.СуммаОборотКт/1000, Истина);
			ИтогоСумма = ИтогоСумма + Выборка.СуммаОборотКт/1000;
		КонецЦикла;
		СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Итого'");
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ИтогоСумма, Ложь);
		...
		Для Каждого ДопПараметр Из СтрРазделить(НСтр("ru = 'Овес,Пшеница,Ячмень,Прочие,Итого'"),",") Цикл
			...
			Если СтрокиВыручки.Количество() > 0 Тогда
				ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, СтрокиВыручки[0].РезультатВычисления, Истина);
			КонецЕсли;
			Если СтрокиСебестоимость.Количество() > 0 Тогда
				ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, - СтрокиСебестоимость[0].РезультатВычисления, Истина);
			КонецЕсли;
		КонецЦикла;
		...
		КультурыСено  = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Культура: Сено"); 
		КультурыСенаж = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Культура: Сенаж"); 
		КультурыСилос = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Культура: Силос");
		 
		НоменклатурыСено  = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Номенклатура: Сено"); 
		НоменклатурыСенаж = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Номенклатура: Сенаж"); 
		НоменклатурыСилос = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Номенклатура: Силос");
		 
		НомГруппаСено  = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Ном группа: Сено"); 
		НомГруппаСенаж = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Ном группа: Сенаж"); 
		НомГруппаСилос = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Ном группа: Силос"); 
		//		Выборка = Справочники.схСтруктураПосевныхПлощадей.Выбрать();
		//		ИтогоПлощадь = 0;
		//		
		//		Пока Выборка.Следующий() Цикл
		...
		//			СтруктураСтроки.ДатаОкончания       = ДатаОкончания;
		//			
		//			Если КультурыСено.Найти(Выборка.Культура) <> Неопределено Тогда
		//				СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Сено'");
		//			ИначеЕсли КультурыСенаж.Найти(Выборка.Культура) <> Неопределено Тогда
		//				СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Сенаж'");
		//			ИначеЕсли КультурыСилос.Найти(Выборка.Культура) <> Неопределено Тогда
		...
		|	схВыпускПродукцииСПоляПродукция.Количество КАК Количество,
		|	схВыпускПродукцииСПоляПродукция.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
		|	схВыпускПродукцииСПоляПродукция.ПлощадьПоля КАК ПлощадьПоля,
		|	схВыпускПродукцииСПоляПродукция.Ссылка.Дата КАК Дата
		|ИЗ
		|	Документ.схВыпускПродукцииСПоля.Продукция КАК схВыпускПродукцииСПоляПродукция
		|ГДЕ
		...
		Пока Выборка.Следующий() Цикл                  
			...
			Если НоменклатурыСено.Найти(Выборка.Продукция) <> Неопределено Тогда
				СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Сено'");
			ИначеЕсли НоменклатурыСенаж.Найти(Выборка.Продукция) <> Неопределено Тогда
				СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Сенаж'");
			ИначеЕсли НоменклатурыСилос.Найти(Выборка.Продукция) <> Неопределено Тогда
				СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Силос'");
			Иначе
				СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Прочие'");
			КонецЕсли;	
		...
		СтруктураСтроки.ДатаОкончания       = ДатаОкончания;
		СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Итого'");
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, КоличествоИтого, Ложь);
		...
		Пока Выборка.Следующий() Цикл
			...
			Если НомГруппаСено = Выборка.Продукция Тогда
				СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Сено'");
			ИначеЕсли НомГруппаСенаж = Выборка.Продукция Тогда
				СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Сенаж'");
			ИначеЕсли НомГруппаСилос = Выборка.Продукция Тогда
				СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Силос'");
			Иначе
				СтруктураСтроки.ДопПараметр  = НСтр("ru = 'Прочие'");
			КонецЕсли;	
		...
		Для Каждого ДопПараметр Из СтрРазделить(НСтр("ru = 'Сено,Сенаж,Силос,Прочие,Итого'"),",") Цикл
			...
			ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, 0, Истина);
		...
		Если СтрокиЗерноводства.Количество() > 0 Тогда
			ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, СтрокиЗерноводства[0].РезультатВычисления, Истина);
		КонецЕсли;
		Если СтрокиКормозаготовки.Количество() > 0 Тогда
			ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, СтрокиКормозаготовки[0].РезультатВычисления, Истина);
		КонецЕсли;
