...
Процедура ОтладкаЖивотноводство2019(Объект, НачальнаяТаблица, СтруктураСтроки, ВыборкаПолейШаблона) Экспорт
	...
	Для НомерГода = 2019 По 2020 Цикл
		...
		КРС = ЕАС_ЗаполнениеОтчетов.ЗначениеПараметраЗаполнения(Объект, "КРС");
		...
		ЗапросСП51.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		                   ...
		                   |				И Субконто1 В ИЕРАРХИИ (&КРС)) КАК ХозрасчетныйОстаткиИОбороты
		                   |ГДЕ
		                   |	&СкладскойУчет = 0
		                   |	И (&ВариантОтчета = 0
		                   |			ИЛИ &ВариантОтчета = 2)
		                   ...
		                   |	(&ВариантОтчета = 1
		                   |			ИЛИ &ВариантОтчета = 2)
		                   |	И ВЫРАЗИТЬ(ЗначенияСвойствОбъектов.Значение КАК Справочник.Номенклатура) В ИЕРАРХИИ (&КРС)
		                   |;
		                   |
		                   |////////////////////////////////////////////////////////////////////////////////
		                   |ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		ЗапросСП51.УстановитьПараметр("КРС", КРС);
		...
		ЗапросСП51.УстановитьПараметр("Корова", Справочники.Номенклатура.НайтиПоНаименованию("Корова", Истина, КРС));    
		РезультатЗапроса = ЗапросСП51.Выполнить().Выбрать();
		Если РезультатЗапроса.Следующий() Тогда
		...
		ПоказателиАПК.Свойство("Форма13АПК_стр131110_гр3", СреднегодовоеПоголовьеКоров);
		...
		МассивКоровы.Добавить(Справочники.Номенклатура.НайтиПоНаименованию("Корова", Истина, КРС));
		МассивКоровы.Добавить(Справочники.Номенклатура.НайтиПоНаименованию("Нетели", Истина, КРС));
		...
		Запрос.УстановитьПараметр("Молоко",        Справочники.Номенклатура.НайтиПоНаименованию("Молоко"));
