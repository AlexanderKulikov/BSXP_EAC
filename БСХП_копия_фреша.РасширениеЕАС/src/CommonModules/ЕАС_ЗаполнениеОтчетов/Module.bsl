...
Процедура ОтладкаЖивотноводство2019(Объект, НачальнаяТаблица, СтруктураСтроки, ВыборкаПолейШаблона) Экспорт
	...
	Для НомерГода = 2019 По 2020 Цикл
		...
		МолоднякКРС2019 = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Номенклатурная Молодняк КРС 2019"); 
		МолоднякКРС2020 = ЕАС_ЗаполнениеОтчетовКопируемый.ЗначениеПараметраЗаполнения(Объект, "Номенклатурная Молодняк КРС 2020");
		...
		Пока РезультатЗапроса.Следующий() Цикл
			Если (НомерГода = 2019 И
					МолоднякКРС2019.Найти(РезультатЗапроса.Номенклатура) <> Неопределено)
					               ИЛИ
				 (НомерГода = 2020 И
					МолоднякКРС2020.Найти(РезультатЗапроса.Номенклатура) <> Неопределено)
					                Тогда
		...
		ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ИтогоРеализованоМясаЛошадей/100, Ложь);
		...
		СтрокаВыручки = ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ИтогоВыручкаМясаЛошадей/1000, Ложь);
		...
		СтрокаСебестоимость = ЕАС_ЗаполнениеОтчетовКопируемый.УстановитьРезультатВычисления(НачальнаяТаблица, СтруктураСтроки, ИтогоСебестоимостьМясаКонины / 1000, Ложь);
