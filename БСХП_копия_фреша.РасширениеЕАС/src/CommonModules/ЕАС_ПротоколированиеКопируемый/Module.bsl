...
//
Функция НачатьФормированиеПротокола(ВидПротокола, Протокол, УзелОбмена = Неопределено) Экспорт
	...
	Если ЗначениеРазделителяСеанса > 0 Тогда
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Начат обмен данными %1. Узел - %2. Область -  %3. Текущее время - %4'"),
									СтруктураПротокол.ВидПротокола,
									?(УзелОбмена = Неопределено, "Не определен", УзелОбмена),
									ЗначениеРазделителяСеанса,
									СтруктураПротокол.НачалоОбмена);
    Иначе
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Начат обмен данными %1. Узел - %2. Разделение не установлено. Текущее время - %4'"),
									СтруктураПротокол.ВидПротокола,
									?(УзелОбмена = Неопределено, "Не определен", УзелОбмена),
									Неопределено,
									СтруктураПротокол.НачалоОбмена);
...
Процедура ЗакончитьФормированиеПротокола(СтруктураПротокол) Экспорт
	...
	ПротоколМенеджер.УзелОбмена       = СтруктураПротокол.УзелОбмена;
