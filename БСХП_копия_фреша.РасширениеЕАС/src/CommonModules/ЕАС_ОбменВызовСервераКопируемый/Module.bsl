...
Функция СформироватьXMLСообщениеЕАС(УзелОбмена, ТекстОшибки, СтруктураПротокол) Экспорт
	...
	Попытка
		...
		Попытка
			...
			ИзмененияОбъект.RegionTo     = ОбластьКуда;
...
Процедура ПрочитатьXMLСообщениеЕАС(ДанныеXML, ТекстОшибки, СтруктураПротокол, ДанныеСообщения = Неопределено) Экспорт
	...
	Попытка 
		...
		Если ТекущаяОбласть <> ИзмененияОбъект.RegionTo Тогда
			...
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
		ТекущаяОбласть_Там = ИзмененияОбъект.RegionFrom;			
			
		Если РазделениеВключено И ТекущаяОбласть = 0 Тогда
			ТекстОшибки =  НСтр("ru = 'Используется обмен с разделенной ИБ без установки области данных.'");
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
		...
		СтруктураПротокол.Вставить("УзелОбмена", УзелОбмена);
		...
		Для Каждого СтрокаСсылок Из ТаблицаСсылок Цикл
			Если СтрокаСсылок.Новая Тогда
				Если НЕ СтрокаСсылок.ЭтоУдаление Тогда
					...
					СтруктураЗаписи.Вставить("ОбластьДанныхИсточника",           ТекущаяОбласть);
					СтруктураЗаписи.Вставить("ОбластьДанныхПриемника", 			 ТекущаяОбласть_Там);
		...
		Попытка
				...
				Для Каждого СвойствоДанных Из ДанныеПодсистемыОбъект.Значение.Свойства() Цикл
					...
					Если ТипЗнч(ЗагружаемыеДанныеXDTO) = Тип("ОбъектXDTO") 
						...
						Иначе
							...
							МассивПроверяемыхСсылок = Новый Массив;
							Для Каждого Запись Из ЗагружаемыеДанныеXDTO.Объект.Record Цикл
								...
								Если Менеджер <> Неопределено И СтроковыеФункцииКлиентСервер.ЭтоУникальныйИдентификатор(Запись.УникальныйИдентификаторПриемника) Тогда
									...
									ИсточникСсылка = Менеджер.ПолучитьСсылку(УникальныйИдентификаторИсточника);
									МассивПроверяемыхСсылок.Добавить(ИсточникСсылка);
								КонецЕсли;
							КонецЦикла;
							
							ЗначенияРеквизитаОбъектов = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(МассивПроверяемыхСсылок, "ОбластьДанных");		 
							...
							Для Каждого Запись Из ЗагружаемыеДанныеXDTO.Объект.Record Цикл
								...
								Если Менеджер <> Неопределено И СтроковыеФункцииКлиентСервер.ЭтоУникальныйИдентификатор(Запись.УникальныйИдентификаторПриемника) Тогда
									УникальныйИдентификаторИсточника = Новый УникальныйИдентификатор(Запись.УникальныйИдентификаторПриемника);
									ИсточникСсылка = Менеджер.ПолучитьСсылку(УникальныйИдентификаторИсточника);
									// Могут чужие случайно залететь
									Если ЗначенияРеквизитаОбъектов.Получить(ИсточникСсылка) = ТекущаяОбласть Тогда  
										СтруктураЗаписи.Вставить("УникальныйИдентификаторПриемника", Запись.УникальныйИдентификаторИсточникаСтрокой);
										СтруктураЗаписи.Вставить("УникальныйИдентификаторИсточника", ИсточникСсылка);
			...
			Для Каждого ЗагружаемыйОбъект Из ЗагружаемыеОбъекты Цикл
				...
				///  Установить область зараннее
				МетаданныеОбъекта = ЗагружаемыйОбъект.Метаданные();
				Если ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(МетаданныеОбъекта) И
						МетаданныеОбъекта.Реквизиты.Найти("ОбластьДанных") <> Неопределено Тогда
					Если ОбщегоНазначения.СсылкаСуществует(ЗагружаемыйОбъект.Ссылка) Тогда
						СтараяОбластьДанных = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗагружаемыйОбъект.Ссылка, "ОбластьДанных");
						Если СтараяОбластьДанных <> ТекущаяОбласть Тогда
							ТекстОшибки = СтрШаблон(НСтр("ru = '%1 Попытка изменения области. Установленная ранее область - %2, текущая область - %3.'"),
							ЗагружаемыйОбъект,
							СтараяОбластьДанных,
							ТекущаяОбласть);
							ВызватьИсключение ТекстОшибки;	
						КонецЕсли;	
					КонецЕсли;
					ЗагружаемыйОбъект.ОбластьДанных = ТекущаяОбласть;
				КонецЕсли;
				...
				ПроверитьОбластиОбъекта(ЗагружаемыйОбъект, ТекущаяОбласть);
			КонецЦикла;
			...
			ПроверитьОбластиИсточников(СоответствияОбъектовИнформационныхБаз, ТекущаяОбласть);
...
Процедура ПроверитьОбластиОбъекта(ПроверяемыйОбъект, ОбластьДанных)

	ТекстОшибки = "";
	
	ВсеТипыЕАС = Новый Массив;
	Для Каждого МетаданныеДанные Из Метаданные.Документы Цикл
		Если Лев(МетаданныеДанные.Имя, 4) = "ЕАС_" Тогда
			ВсеТипыЕАС.Добавить(Тип("ДокументСсылка."+МетаданныеДанные.Имя));
		КонецЕсли;
	КонецЦикла;		  
	Для Каждого МетаданныеДанные Из Метаданные.Справочники Цикл
		Если Лев(МетаданныеДанные.Имя, 4) = "ЕАС_" Тогда
			ВсеТипыЕАС.Добавить(Тип("СправочникСсылка."+МетаданныеДанные.Имя));
		КонецЕсли;
	КонецЦикла;		  
	
	МетаданныеОбъекта = ПроверяемыйОбъект.Метаданные();
	
	Если ОбщегоНазначения.ЭтоДокумент(МетаданныеОбъекта) Или
	 		ОбщегоНазначения.ЭтоСправочник(МетаданныеОбъекта) Тогда
		ДопРеквизиты = Новый Массив; 	 			 
	 	...
	 	ИначеЕсли ОбщегоНазначения.ЭтоСправочник(МетаданныеОбъекта) Тогда
	 		...
	 		Если МетаданныеОбъекта.Владельцы.Количество() > 0 Тогда
	 			ДопРеквизиты.Добавить("Владелец");
	 		КонецЕсли;	
	 		Если МетаданныеОбъекта.Иерархический Тогда
	 			ДопРеквизиты.Добавить("Родитель");
	 		КонецЕсли;	
	 	КонецЕсли;		
	 	ЗапросДляОбъекта = "";
	 	
 		ЗапросДляРеквизитовМетаданных(МетаданныеОбъекта.Реквизиты, ИмяТаблицы, ЗапросДляОбъекта, ВсеТипыЕАС, ДопРеквизиты);
 		
		Для Каждого ТабличнаяЧасть Из МетаданныеОбъекта.ТабличныеЧасти Цикл
			ЗапросДляРеквизитовМетаданных(ТабличнаяЧасть.Реквизиты, ИмяТаблицы + "." + ТабличнаяЧасть.Имя, ЗапросДляОбъекта, ВсеТипыЕАС);
		КонецЦикла;
			
		Если ЗначениеЗаполнено(ЗапросДляОбъекта) Тогда
			Запрос = Новый Запрос(ЗапросДляОбъекта);
		...
		Пока Выборка.Следующий() Цикл
			ТекущаяОшибка = СтрШаблон(Нстр("ru = '%1:  %2 - %3 - ссылается на область %4 отличающуюся от текущей %5'"),
										ПроверяемыйОбъект,
										Выборка.ИмяПоля.
										Выборка.Поле,
			...
			Если ЗначениеЗаполнено(ТекстОшибки) Тогда
				...
				|" + ТекущаяОшибка;
			Иначе								
				ТекстОшибки = ТекущаяОшибка;
			КонецЕсли;	
		КонецЦикла;	
	ИначеЕсли ОбщегоНазначения.ЭтоРегистрСведений(МетаданныеОбъекта) 
			И ТипЗнч(ПроверяемыйОбъект) = Тип("РегистрСведенийНаборЗаписей.ЕАС_ПрисоединенныеДанные") Тогда
		Для Каждого Запись Из ПроверяемыйОбъект Цикл		
			Запрос = Новый Запрос;
			Запрос.Текст =
					"ВЫБРАТЬ
					|	ПрисоединенныеДанные.ВладелецДанных,
					|	ПрисоединенныеДанные.ВидДанных,
					|   ПрисоединенныеДанные.ВладелецДанных.ОбластьДанных КАК ОбластьДанных
					|ИЗ
					|	РегистрСведений.ЕАС_ПрисоединенныеДанные КАК ПрисоединенныеДанные
					|ГДЕ
					|	ПрисоединенныеДанные.ВладелецДанных.ОбластьДанных <> &ОбластьДанных
					|	И (ПрисоединенныеДанные.ВидДанных = &ВидДанных
					|	И ПрисоединенныеДанные.ВладелецДанных = &ВладелецДанных)";
				
			Запрос.УстановитьПараметр("ОбластьДанных",   ОбластьДанных);
			Запрос.УстановитьПараметр("ВидДанных",		 Запись.ВидДанных);
			Запрос.УстановитьПараметр("ВладелецДанных",  Запись.ВладелецДанных);
				
			Выборка = Запрос.Выполнить().Выбрать();
				
			Пока Выборка.Следующий() Цикл
				ТекущаяОшибка = СтрШаблон(Нстр("ru = '%1: ВидДанных - %2,  ВладелецДанных - %3 - ссылается на область %4 отличающуюся от текущей %5'"),
											ПроверяемыйОбъект,
											Запись.ВидДанных,
											Запись.ВладелецДанных,
											Выборка.ОбластьДанных,
											ОбластьДанных);
				Если ЗначениеЗаполнено(ТекстОшибки) Тогда
					ТекстОшибки = ТекстОшибки + "
...
Процедура ЗапросДляРеквизитовМетаданных(ОбъектМетаданныхРеквизиты, ИмяТаблицы, ТекстЗапроса, ВсеТипыЕАС, ДопРеквизиты = Неопределено)
	
 	МассивРеквизитов = Новый Массив();		
	Для Каждого Реквизит Из ОбъектМетаданныхРеквизиты Цикл
		Для Каждого ТипЕАС Из ВсеТипыЕАС Цикл
			Если Реквизит.Тип.СодержитТип(ТипЕАС) Тогда
				МассивРеквизитов.Добавить(Реквизит.Имя);
			КонецЕсли;	
		КонецЦикла;	
		Если ТипЗнч(ДопРеквизиты) = Тип("Массив") Тогда
			Для Каждого ДопРеквизит Из ДопРеквизиты Цикл
				МассивРеквизитов.Добавить(ДопРеквизит);
			КонецЦикла;	
		КонецЕсли;	 
	КонецЦикла;
	
	Если МассивРеквизитов.Количество() > 0 Тогда
		Для Сч = 0 По МассивРеквизитов.Количество() - 1 Цикл
			ИмяРеквизита = МассивРеквизитов[Сч];
			...
			ТекстЗапроса = ТекстЗапроса + "ВЫБРАТЬ 
				|	ОбъектЕАС." + ИмяРеквизита + " КАК Поле,
				|   """ + ИмяРеквизита + """ КАК ИмяПоля,
				|   ОбъектЕАС." + ИмяРеквизита + ".ОбластьДанных КАК ОбластьДанных
				|ИЗ
				|	" + ИмяТаблицы + " КАК ОбъектЕАС
				|ГДЕ
				|	ОбъектЕАС.Ссылка = &Ссылка
				|И ОбъектЕАС." + ИмяРеквизита + ".ОбластьДанных <> &ОбластьДанных";
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры

Процедура ПроверитьОбластиИсточников(СоответствияОбъектовИнформационныхБаз, ОбластьДанных)
	ТекстОшибки = "";
	
	МассивСсылок = Новый Массив;
	Для Каждого СтруктураЗаписи Из СоответствияОбъектовИнформационныхБаз Цикл
		МассивСсылок.Добавить(СтруктураЗаписи.УникальныйИдентификаторИсточника);
	КонецЦикла;
	
	ЗначенияРеквизитов = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(МассивСсылок, "ОбластьДанных");
	Для Каждого КлючЗначение Из ЗначенияРеквизитов Цикл
		Если КлючЗначение.Значение <> ОбластьДанных Тогда
			ЛокальныйТекстОшибки = СтрШаблон(НСтр("ru = 'Ошибка установки соответветствия. Объект -%1, Область источника %2.  Текущая область - %3'"),
					 КлючЗначение.Ключ,
			...
			Иначе
				ТекстОшибки = ЛокальныйТекстОшибки;	
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ОпределитьЗначениеСправочника(ИмяПоля, ДанныеПоля)
	...
	Если ИмяПоля = "ЕдиницаИзмерения" Тогда
		Если ЗначениеЗаполнено(ДанныеПоля.Код) Тогда
