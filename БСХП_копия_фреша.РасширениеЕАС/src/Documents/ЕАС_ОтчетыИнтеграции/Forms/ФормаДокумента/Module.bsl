...
Процедура Заполнить(Команда)
	...
	Если ЗначениеЗаполнено(Объект.Шаблон) Тогда
		...
		// Возможно период был установлен при заполнении             
		ПериодОтчета.ДатаОкончания = Объект.ДатаОкончания;
		ПериодОтчета.ДатаНачала    = Объект.ДатаНачала;
	Иначе
...
Процедура УстановитьПараметрыЗаполненияПоУмолчанию()
	ТЗПараметры = Объект.ПараметрыЗаполнения.Выгрузить();     
	...
	Для Каждого ПараметрЗаполненияШаблон Из Объект.Шаблон.ПараметрыЗаполнения Цикл
		...
		Если Не ЗначениеЗаполнено(ЗначениеВФорме) Тогда
			// Ищем ранее установленное значение 
			Отбор = Новый Структура("Имя,Тип,Список");
			ЗаполнитьЗначенияСвойств(Отбор, ПараметрЗаполненияШаблон);
			НайденныеСтроки = ТЗПараметры.НайтиСтроки(Отбор);
			Если НайденныеСтроки.Количество() > 0 Тогда
				ЗначениеВФорме = НайденныеСтроки[0].ЗначениеВФорме;
			КонецЕсли;	 
		КонецЕсли;
...
Процедура ЗаполнитьПоШаблону()          
	...
	ЕАС_ЗаполнениеОтчетовКопируемый.ВыполнитьАлгоритмБезопасно(Объект, Алгоритмы.АлгоритмНачальногоЗаполненияШаблона, ВыборкаПолейШаблона, НачальнаяТаблица, СтруктураСтроки);
				...
				Если ОписаниеОбласти <> Неопределено Тогда
					...
					Если ИмяПараметра <> Неопределено Тогда
						...
						Если ЗначениеЗаполнено(АлгоритмРасчетаЯчейки) Тогда
							...
							ЕАС_ЗаполнениеОтчетовКопируемый.ВыполнитьАлгоритмБезопасно(Объект, АлгоритмРасчетаЯчейки, ВыборкаПолейШаблона, НачальнаяТаблица, СтруктураСтроки, РезультатРасчета);
	...
	ЕАС_ЗаполнениеОтчетовКопируемый.ВыполнитьАлгоритмБезопасно(Объект, Алгоритмы.АлгоритмЗавершенияЗаполненияШаблона, ВыборкаПолейШаблона, НачальнаяТаблица, СтруктураСтроки, ВыборкаПолейШаблона);
